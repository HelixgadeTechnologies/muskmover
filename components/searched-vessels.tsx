    ...
    <section className="py-20 px-4 bg-white overflow-hidden">
      <div className="max-w-7xl mx-auto">
        ...
        {isLoading ? (
          ...
          <>
            {/* Grid */}
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-12 min-h-[400px]">
              {filteredVessels.length > 0 ? (
                filteredVessels.map((vessel, i) => (
                  <ScrollReveal key={`${activeTab}-${vessel.id}`} delay={0.2 + (i % 4) * 0.1}>
                    <div className="group flex flex-col bg-[#050B20] rounded-none overflow-hidden transition-all hover:shadow-2xl h-full border border-slate-800">
                      {/* Image Section */}
